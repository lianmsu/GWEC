...
special_gene=CBASS__AG_E1_ThiF

source activate defensefinder2.0.0
output=/home/lipengwei2024phd/03GW_eu/40eu_contig/52eucontiggene_secondaryHMM/${special_gene}
cd ${output}

euproteins_path=/home/lipengwei2024phd/database/EukProt/all993eukaryotic_species_proteins.faa

hmmsearch --tblout eu993_${special_gene}_homologs.txt -E 1e-5 ${special_gene}.hmm ${euproteins_path}
...
sed "s/ \+/\t/g" eu993_${special_gene}_homologs.txt | sed "/^#/d" | awk -F"\t" '$8 < 1e-5'| awk 'BEGIN {print "target name\taccession\tquery name\taccession\tE-value\tscore\tbias\tE-value\tscore\tbias\texp\treg\tclu\tov\tenv\tdom\trep\tinc\tdescription of target"} {print}' > eu993_${special_gene}_homologs2.txt

#按照E-value增序排列
#cd /home/lipengwei2024phd/03GW_eu/70selected_DS_phylogenetic/2hmmsearch
(head -n 1 eu993_${special_gene}_homologs2.txt && tail -n +2 eu993_${special_gene}_homologs2.txt | sort -k8,8g) > eu993_${special_gene}_homologs2_eval_increase.txt
#提取faa序列
#cd /home/lipengwei2024phd/03GW_eu/70selected_DS_phylogenetic/2hmmsearch
