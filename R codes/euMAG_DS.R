...
DF_HMM_res <- read.table("DF_hmmer_extract.txt", header = T , check.names = F)
...
HMM_uniref <- read.csv("DF_hmmer_UniRef90_cleaned.tsv", header = T ,sep = '\t',
                       check.names = F)
...
HMM_uniref_merged <- HMM_uniref_merged[!duplicated(HMM_uniref_merged$UniRef90_ID), ]
...
DF_HMM_res <- DF_HMM_res %>%
  mutate(uniref90id = str_extract(hit_id, "^[^|]+"))
...
DF_HMM_res_DG <- table(DF_HMM_res$gene_name) %>% as.data.frame() %>% arrange(desc(Freq))

# 将 Var1 列按照 Freq 列的值从大到小重新排序
DF_HMM_res_DG$Var1 <- reorder(DF_HMM_res_DG$Var, -DF_HMM_res_DG$Freq)

DF_HMM_res_DG1 <- DF_HMM_res_DG[DF_HMM_res_DG$Freq>=10,]
...
DF_HMM_res_DG2 <- DF_HMM_res_DG[DF_HMM_res_DG$Freq<10,]
p2 <- 
  ...
  scale_y_continuous(limits = c(0, 10), expand = c(0, 0)) +  # 设置 y 轴范围和扩展
  labs(x = "", y = "Gene numbers") +  # 设置轴标签
  theme_bw() +
  theme(
    plot.margin = unit(c(0.2, 0.2, 0.2, 0.2), "cm"),  # 设置图形边距
...
DF_HMM_res_uniprot90_Descript_G1 <- DF_HMM_res_uniprot90_Descript[DF_HMM_res_uniprot90_Descript$Freq>=15,]
...
p3 <- 
  ...
  scale_y_continuous(limits = c(0, 250), expand = c(0, 0)) +  # 设置 y 轴范围和扩展
  labs(x = "", y = "Gene numbers") +  # 设置轴标签
  theme_bw() +
  theme(
    plot.margin = unit(c(0.2, 0.2, 0.2, 0.2), "cm"),  # 设置图形边距
