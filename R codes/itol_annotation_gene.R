...
setwd("D:/pku/01groundwater/02eukaryotes/70selected_gene_phylo")
# 假设您的树存储在一个名为"tree"的变量中
tree <- "SngC_bac_eu_600.super.trim.tree"
...
sngc_gene <- read.delim2("SngC_merged_file.id", header = T)


#按门设置颜色
library(dplyr)
df_phycolor <- sngc_gene %>% select(seqname, source)
set.seed(666)
setwd("D:/pku/01groundwater/02eukaryotes/70selected_gene_phylo")
...
#
df_phycolor <- merge(df_phycolor,element_count_sorted,by.x = "source",by.y = "source")
df_phycolor <- df_phycolor[, c("seqname", "source","color")]
df_phycolor$seqname <- sub("\\[.*", "", df_phycolor$seqname)
df_phycolor <- df_phycolor[df_phycolor$seqname %in% phylo[["tip.label"]],]

df_phycolor$range <- 'range'
df_phycolor <- df_phycolor[, c("seqname",'range', "color","source")]
write.table(df_phycolor,"df_phycolor_600.txt",quote = F,row.names = F,sep = '\t')

#subtype = "range"才能大面积改变颜色
unit_2 <- create_unit(data = df_phycolor, 
                      key = "1sngc_Hits_source_color_range", 
                      type = "TREE_COLORS", 
                      subtype = "range", 
