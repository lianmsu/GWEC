...
setwd("D:/pku/01groundwater/02eukaryotes/51euMAG_eu2eu")
eu2eu_HMM_res <- read.table("best_hits_eu2eu_3.txt", sep = '\t', header = T , check.names = F)


#1.2. 对HMM结果的Uniref90注释结果分析#####################
setwd("D:/pku/01groundwater/02eukaryotes/51euMAG_eu2eu")

uniref_anno <- read.csv("UniRef90annotation_cleaned.tsv", header = T ,sep = '\t',
                       check.names = F)
...
#把物种结果合并回uniref_anno
# 合并操作
uniref_anno_merged <- uniref_anno %>%
  left_join(tax_id, by = c("TaxID" = "taxid"))

# 确保HMM_uniref_merged中的UniRef90_ID是唯一的
uniref_anno_merged <- uniref_anno_merged[!duplicated(uniref_anno_merged$UniRef90_ID), ]

#只保留HMM_uniref_merged的last_one的值为2759
uniref_anno_merged <- uniref_anno_merged[uniref_anno_merged$last_two == 2759,]

#这里是统计所有对应到的Uniref90的类别。非冗余！！！
uniref_anno_protein <- table(uniref_anno_merged$Description) %>% as.data.frame()

viral_rows <- uniref_anno_protein[grepl("viral", uniref_anno_protein$Var1, ignore.case = TRUE), ]
print(viral_rows)

#可以绘制一个图展示非冗余Uniref90的组成

#合并uniref_anno_merged与eu2eu_HMM_res

#提取eu2eu_HMM_res的hit_id列的|前的字符串部分，作为新列uniref90id
# 检查是否有重复列名
any(duplicated(colnames(eu2eu_HMM_res)))
# 修复重复列名
colnames(eu2eu_HMM_res) <- make.unique(colnames(eu2eu_HMM_res))
# 使用 str_extract 提取 | 前的部分
eu2eu_HMM_res <- eu2eu_HMM_res %>%
  mutate(uniref90id = str_extract(`target name`, "^[^|]+"))

# 合并数据框
eu2eu_HMM_res <- merge(eu2eu_HMM_res, uniref_anno_merged[, c("UniRef90_ID", "Description")], 
...
eu2eu_HMM_res <- eu2eu_HMM_res[!is.na(eu2eu_HMM_res$Description), ]


#HMM结果不适合以DS角度去画图描述，因为都是单个基因结果。
eu2eu_HMM_res_gene <- table(eu2eu_HMM_res$`query name`) %>% as.data.frame() %>% arrange(desc(Freq))

# 将 Var1 列按照 Freq 列的值从大到小重新排序
eu2eu_HMM_res_gene$Var1 <- reorder(eu2eu_HMM_res_gene$Var, -eu2eu_HMM_res_gene$Freq)

eu2eu_HMM_res_gene1 <- eu2eu_HMM_res_gene[eu2eu_HMM_res_gene$Freq>=350,]
...
p <- 
ggplot(eu2eu_HMM_res_gene1, aes(x = Var1, y = Freq)) +
  geom_bar(
    stat = "identity", position = "dodge",
    show.legend = TRUE, alpha = 0.9, linewidth = 0.5, fill = "#C39398"  # 设置柱子颜色
  ) +
  geom_text(
  ...
  scale_y_continuous(limits = c(0, 1650), expand = c(0, 0)) +  # 设置 y 轴范围和扩展
...
ggsave("euMAG_DS.pdf",plot = p, width = 5.9, height = 6.2, units = 'in')
...
#HMM结果不适合以DS角度去画图描述，因为都是单个基因结果。
eu2eu_HMM_res_uniprot90_Descript <- table(eu2eu_HMM_res$Description) %>% as.data.frame() %>% arrange(desc(Freq))

# 将 Var1 列按照 Freq 列的值从大到小重新排序
eu2eu_HMM_res_uniprot90_Descript$Var1 <- reorder(eu2eu_HMM_res_uniprot90_Descript$Var1, -eu2eu_HMM_res_uniprot90_Descript$Freq)

eu2eu_HMM_res_uniprot90_Descript <- eu2eu_HMM_res_uniprot90_Descript[eu2eu_HMM_res_uniprot90_Descript$Freq>=150,]
...
p3 <- 
  ggplot(eu2eu_HMM_res_uniprot90_Descript, aes(x = Var1, y = Freq)) +
  geom_bar(
    stat = "identity", position = "dodge",
  ...
  scale_y_continuous(limits = c(0, 5000), expand = c(0, 0)) +  # 设置 y 轴范围和扩展
  labs(x = "", y = "Gene numbers") +  # 设置轴标签
  theme_bw() +
  theme(
    plot.margin = unit(c(0.2, 0.2, 0.2, 0.2), "cm"),  # 设置图形边距
...
library(Cairo)
library(cowplot)
cairo_pdf(filename = "euMAG_HMM_eu2eu_all.pdf" , width = 9,height = 6.68)
g <- plot_grid(p,p3,
...
library(stringr)
# 提取 | 分隔的第二部分，并去掉末尾的 _ 和数字
eu2eu_HMM_res <- eu2eu_HMM_res %>%
  mutate(
    genome = str_extract(`target name`, "(?<=[|])[^|]+(?=[|])"),  # 提取 | 分隔的第二部分
    genome = str_replace(genome, "_[0-9]+$", "")               # 去掉末尾的 _ 和数字
  )

eu2eu_HMM_res_bin <- table(eu2eu_HMM_res$genome) %>% as.data.frame() 

eu2eu_HMM_res_bin <- merge(eu2eu_HMM_res_bin,euMAG_tax,by.x = 'Var1',by.y = 'bin')

# 绘制柱状图
ggplot(eu2eu_HMM_res_bin, aes(x = clade2, y = Freq)) +
